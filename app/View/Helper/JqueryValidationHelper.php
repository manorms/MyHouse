...
class JqueryValidationHelper extends AppHelper {
  ...
  public function input($fieldName, $options = array()) {
    ...
    $jquery_validate = strtr(json_encode($meta, JSON_NUMERIC_CHECK), $boolean_fix_map);
	if( trim($jquery_validate) != '[]' )$options['data-validate'] = $jquery_validate;    
	$response = $this->_addJs($map);
	$response .= $this->Form->input($fieldNameOriginal, $options);
	return $response;
  }
		  ...
		  foreach ($validateItem as $validateName => $validateParams) {
			if (!empty($validateParams['rule'])) {
			  ...
			  if (method_exists($this, $methodName)) {
				$meta = array_merge_recursive ($meta, $this->$methodName($model, $field, $rule, $msg));
