 ...
 */
	public function beforeSave(Model $model) {
		...
		foreach ($this->settings[$model->alias] as $field => $options) {
            ...
            if ( isset($options['nameCallback']) && method_exists($model, $options['nameCallback'])){
                $info = pathinfo($this->runtime[$model->alias][$field]['name']);
                $this->runtime[$model->alias][$field]['name'] = $model->{$options['nameCallback']}($model->data[$model->alias][$field]) . ".{$info['extension']}";
            }
            $this->runtime[$model->alias][$field]['name'] = $this->_sanitizeFilename($this->runtime[$model->alias][$field]['name']);
	...
	function beforeValidate(Model $model) {
		...
		//die();
		foreach ($this->settings[$model->alias] as $field => $options) {
			if (isset($model->data[$model->alias][$field]) && $this->_isURI($model->data[$model->alias][$field])) {
	...
	public function afterSave(Model $model, $created) {
		...
		foreach ($this->settings[$model->alias] as $field => $options) {
            ...
            $temp[$model->alias][$field] = "'" . DS . str_replace(WWW_ROOT, "", $filePath) . "'";
 ...
 **/
    ...
    public function _sanitizeFilename($fileName, $defaultIfEmpty = 'default', $separator = '_', $lowerCase = true)
	{
        ...
        $fileName = $this->sanear_string($fileInfos['filename']);
		// Removes accents
		//$fileName = @iconv('UTF-8', "ASCII//TRANSLIT", $fileInfos['filename']);
